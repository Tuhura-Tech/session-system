...
class SessionBlock(UUIDv7AuditBase):
    ...
    __table_args__ = (
        UniqueConstraint("year", "block_type", name="uq_session_blocks_year_type"),
    )

    year: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    block_type: Mapped[SessionBlockType] = mapped_column(
    ...
    occurrences: Mapped[list[SessionOccurrence]] = relationship(
        "SessionOccurrence", back_populates="block"
    )
