...
from app.db.models.types import IntEnumType
...
# [SQL: SELECT sessions.id, sessions.session_location_id, sessions.year, sessions.session_type, sessions.name, sessions.age_lower, sessions.age_upper, sessions.day_of_week, sessions.start_time, sessions.end_time, sessions.waitlist, sessions.capacity, sessions.what_to_bring, sessions.prerequisites, sessions.photo_album_url, sessions.internal_notes, sessions.archived, sessions.sa_orm_sentinel, sessions.created_at, sessions.updated_at, count(*) OVER () AS anon_1
# FROM sessions
...
class Session(UUIDv7AuditBase):
    ...
    session_location_id: Mapped[uuid.UUID] = mapped_column(
        UUID(), ForeignKey("session_locations.id"), nullable=False
    )
    ...
    day_of_week: Mapped[DayOfWeekEnum | None] = mapped_column(
        IntEnumType(DayOfWeekEnum), nullable=True
    )
    ...
    # Relationships
    session_location: Mapped[SessionLocation] = relationship(
        "SessionLocation", back_populates="sessions"
    )
