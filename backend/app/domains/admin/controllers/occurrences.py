...
#                 select(Staff.id).where(Staff.id.in_(data.staff_ids))
#             )
...
#                     detail=f"Staff members not found: {', '.join(str(id) for id in missing_ids)}"
#                 )
...
#         await db.execute(
#             sql_delete(SessionStaff).where(SessionStaff.session_id == session_id)
#         )
...
#         return Response(
#             content={
#                 "message": f"Assigned {len(data.staff_ids)} staff member(s) to session"
#             },
#             status_code=HTTP_200_OK,
#         )
...
from app.domains.admin.schemas.occurrences import Occurrence, OccurrenceCreate, OccurrenceUpdate
...
class OccurrenceController(Controller):
    ...
    async def update_occurrence(
        ...
        occurrence = await occurrence_service.update(occurrence_id, occurrence_data)
        if not occurrence:
            raise NotFoundException(detail="Occurrence not found")
        return occurrence_service.to_schema(occurrence, schema_type=Occurrence)
    ...
    async def create_occurrence(
        ...
        occurrence = await occurrence_service.create(occurrence_data)
        return occurrence_service.to_schema(occurrence, schema_type=Occurrence)