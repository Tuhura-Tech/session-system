...
class AuthController(Controller):
    ...
    async def request_magic_link(
        self, db: AsyncSession, data: MagicLinkRequest
    ...
    async def consume_magic_link(
        self, db: AsyncSession, token: str, returnTo: str | None = None
    ) -> Response:
    ...
    async def logout(self, request: Request, db: AsyncSession) -> Response:
        ...
        if raw:
            ...
            now = utcnow()
            result = await db.execute(
                select(CaregiverSession).where(
                    CaregiverSession.token_hash == token_hash
                )
            )
        ...
        response = Response(
            content=LogoutResponse(ok=True).model_dump(), status_code=HTTP_200_OK
        )
